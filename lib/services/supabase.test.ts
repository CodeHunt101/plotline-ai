...
describe("matchMoviesByEmbedding", () => {
  ...
  it("should return matches when found", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(`${SUPABASE_WORKER_URL}/api/match-movies`, {
      method: "POST",
      headers: {
  ...
  it("should handle network errors", async () => {
    ...
    await expect(matchMoviesByEmbedding(mockEmbedding)).rejects.toThrow("Network error");
  });
