...
async function handleMovieRecommendations(request: Request, openai: OpenAI): Promise<Response> {
...
async function handleEmbeddings(request: Request, openai: OpenAI): Promise<Response> {
  ...
  return new Response(JSON.stringify({ embedding: embeddingResponse.data[0].embedding }), {
    headers: corsHeaders,
  });
}
